 ...
 */            (function(window, factory) {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) new core(".main-page__slider", {
        ...
        function documentFunc() {
            if (menuBody && firstSection) {
                if (window.innerWidth <= 767.98) menuBody.style.paddingTop = 1 * (header.offsetHeight + 40) / 16 + "rem"; else menuBody.style.paddingTop = 0;
                firstSection.style.paddingTop = 1 * header.offsetHeight / 16 + "rem";
            }
            document.querySelector(".catalog-link");
            document.querySelectorAll(".menu__link");
        }
        window.addEventListener("scroll", documentFunc);
        window.addEventListener("resize", documentFunc);
        window.addEventListener("load", documentFunc);
        function filterResponse() {
            const filterBtn = document.querySelectorAll(".catalog-page__item");
            const filterItem = document.querySelectorAll(".catalog-item");
            filterBtn.forEach((button => {
                button.addEventListener("click", filterBtnActions);
                function filterBtnActions() {
                    const buttonCarType = button.dataset.carType;
                    filterBtn.forEach((button => {
                        button.classList.remove("_active");
                    }));
                    button.classList.add("_active");
                    filterItem.forEach((item => {
                        if (item.dataset.carType === buttonCarType || "*" === buttonCarType) item.style.display = ""; else item.style.display = "none";
                    }));
                }
            }));
        }
        function activeLink() {
            ...
            links.forEach((link => {
                if (link.href === currentUrl) link.classList.add("_active");
            }));
        }
